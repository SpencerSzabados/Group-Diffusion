...
def iterative_colorization(
    ...
    Q = th.from_numpy(obtain_orthogonal_matrix()).to(distribute_util.dev()).to(th.float32)
    mask = th.zeros(*x.shape[1:], device=distribute_util.dev())
    mask[0, ...] = 1.0
...
def iterative_inpainting(
    ...
    img_th = th.from_numpy(img_np).to(distribute_util.dev())

    mask = th.zeros(*x.shape, device=distribute_util.dev())
