...
class FIDAndIS:
    ...
    def get_preds(self, batch, captions=None):
        with torch.no_grad():
            ...
            clip_pred = self.clip_visual(clip_in.half().to(distribute_util.dev()))
            if captions is not None:
                text_in = self.clip_tokenizer(captions)
                text_pred = self.clip_text(text_in.to(distribute_util.dev()))
            else:
    ...
    def get_inception_score(
        ...
        for i in range(0, len(activations), self.softmax_batch_size):
            ...
            with torch.no_grad():
                softmax_out.append(
                    self.inception_softmax(torch.from_numpy(acts).to(distribute_util.dev()))
                    .cpu()
                    .numpy()
                )
        ...
        for i in range(0, len(activations), self.clip_score_batch_size):
            ...
            with torch.no_grad():
                image_features = self.clip_proj(
                    torch.from_numpy(acts).half().to(distribute_util.dev())
                )
    ...
    def get_activations(self, data, num_samples, global_batch_size, pr_samples=50000):
        ...
        for _ in tqdm(range(0, int(np.ceil(num_samples / global_batch_size)))):
            ...
            batch, cond = batch.to(distribute_util.dev()), {
                k: v.to(distribute_util.dev()) for k, v in cond.items()
            }
