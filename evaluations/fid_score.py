...
def get_activations(files, model, batch_size=64, dims=2048, img_size =32, device='cpu',
    ...
    assert not (eqv != 'Z2' and gen_op is not None),\
          f'eqv is {eqv} (not Z2). Processing ref dataset and gen_op should be None'
    ...
    if eqv == 'Z2':
        ...
        def grp_op(x):
            return torch.cat([torch.rot90(x, k = k, dims = [-1, -2]) for k in range(4)], dim = 0)
        batch_mult = 0.25
    elif eqv == 'D4':       
        def grp_op(x):
            return torch.cat(
                [torch.rot90(x, k = k, dims = [-1, -2]) for k in range(4)] + \
                [torch.rot90(torch.flip(x, dims = [-2]), k = k, dims = [-1, -2]) for k in range(4)], \
            dim = 0)
        batch_mult = 0.125
    else:
        raise NotImplementedError(f"eqv: {eqv} is not implemented.")

    dl_batch_size = int(batch_size * batch_mult)

    num_samples = int(len(files) / batch_mult)
...
def calculate_eqv_fid_given_paths(path, batch_size, device, dims, num_workers=1, eqv='Z2'):
    ...
    if type(path) == list:
        assert len(path) < 2, f'paths should contain one path, but it contains {len(path)}'
        path = path[0]
    ...
    for i in range(len(mean_var_list)):
        for j in range(i + 1, len(mean_var_list)):
            m_, s_ = mean_var_list[i]
            m__, s__ = mean_var_list[j]
            inv_fid_list.append(calculate_frechet_distance(m_, s_, m__, s__))
