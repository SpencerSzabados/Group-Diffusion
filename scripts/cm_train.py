...
from ..model.utils.train_util import CMTrainLoop
...
def main():
    ...
    if len(args.teacher_model_path) > 0:  # path to the teacher score model.
        ...
        teacher_model.load_state_dict(
            distribute_util.load_state_dict(args.teacher_model_path, map_location="cpu"),
        )

        teacher_model.to(distribute_util.dev())
        teacher_model.eval()
    ...
    target_model.to(distribute_util.dev())
    target_model.train()

    distribute_util.sync_params(target_model.parameters())
    distribute_util.sync_params(target_model.buffers())
